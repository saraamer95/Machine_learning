...
# 
# - While all models are suitable for our data, given the advantages in f-score performance and both time, Adaboost appears to be the best choice.
# 
...
# - Both accuracy and F-score are improved slightly with the optimized model as we can see in the table above. 
# - as we can see from table above the optimizes model better than Unoptimized Model .
# 
...
# 2- Education-num: Better Education means better income.
# 3- Age: Older people are likely to have more work experience or  higher income. 
# 4- Capital-gain: The amount addds to the income. High values would likely increase the possibility that someone earns more than 50k.
# 5- Capital-loss: The capital loss should be also strongly correlated with income
...
# 
#  The final model's F-score and accuracy on the reduced data are lower than the model including all the features. It went down from 0.8608 to 0.8533 for the accuracy and 0.7316 to 0.7222 for F-score. If it was a real time application, I would consider the model on the reduced data. However, in the case of CharityML because the F-score has decreased by more than 5.16%, I would suggest to used the model built based on the entire dataset.
# 
